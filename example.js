(function(globals) {
    ...
    Ext.Loader.setPath('Ext.ux', 'lib/ext-4.0.7/examples/ux/');
    ...
    Ext.onReady(function() {
		...
		// create the Grid
		grid = Ext.create('bg.Ext.CopyPasteGrid', {
			store: store,
			stateful: true,
			collapsible: true,
			stateId: 'stateGrid',
				...
				},
				{
					...
					items: [{
						...
						handler: function(grid, rowIndex, colIndex) {
							...
							globals.alert("Sell " + rec.get('company'));
						}
					}, {
						...
						handler: function(grid, rowIndex, colIndex) {
							var rec = store.getAt(rowIndex);
							globals.alert((rec.get('change') < 0 ? "Hold " : "Buy ") + rec.get('company'));
						}
					}]
				}
			],
